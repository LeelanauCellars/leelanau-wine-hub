...
function distributionPermalinkSlug(item: DistributionWine) {
  return routeSlug(`${item.name}${item.specs.size ? ` ${item.specs.size}` : ''}`);
}

function centralPath(path = '/') {
  const clean = path === '/' ? '' : `/${path.replace(/^\/+|\/+$/g, '')}`;
  return `${CENTRAL_PATH_PREFIX}${clean}` || '/';
}

function wineProfilePath(wine: WineRecord, tab: ProfileTab = 'overview') {
  const suffix = tab === 'overview' ? '' : `/${tab}`;
  return centralPath(`/wine-library/${winePermalinkSlug(wine)}${suffix}`);
}

function techSheetPath(wine: WineRecord) {
  return centralPath(`/tech-sheets/${winePermalinkSlug(wine)}`);
}

function distributionWinePath(item: DistributionWine) {
...
function parseCentralRoute(pathname: string): CentralRoute | null {
  const segments = pathname.split('/').filter(Boolean);
  const routeIndex = segments.findIndex((segment) => CENTRAL_ROUTE_SEGMENTS.has(segment));
  if (routeIndex < 0) return null;
  const route = segments.slice(routeIndex);
...
function writeCentralPath(path: string, replace = false) {
...
function routeForView(view: SectionView): CentralRoute {
...
function CopyLinkButton({ path, label = 'Copy link', className = '' }: { path: string; label?: string; className?: string }) {
  ...
  return <button type="button" onClick={() => void copy()} className={className || 'flex items-center gap-2 rounded-xl border border-black/10 bg-white px-4 py-2.5 text-sm font-bold text-black/65 shadow-sm hover:border-black/20 hover:text-black'}><ExternalLink className="h-4 w-4" /> {copied ? 'Link copied' : label}</button>;
}
...
export default function WineHub() {
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      setView('profile');
      const route: CentralRoute = { kind: 'wine', slug: winePermalinkSlug(match), tab: 'overview' };
      setPendingRoute(route);
      writeCentralPath(wineProfilePath(match), true);
    }
  }, [wines]);

  useEffect(() => {
    if (entryStage !== 'hub' || !portalRole || !pendingRoute) return;
    applyRouteState(pendingRoute, portalRole);
  }, [entryStage, portalRole, pendingRoute, wines, distributionWines]);
  ...
  function applyRouteState(route: CentralRoute, role: PortalRole) {
    ...
    if (route.kind === 'distribution') {
      if (!(role === 'admin' || role === 'sales' || role === 'distribution')) return;
      const item = distributionWines.find((wine) => distributionPermalinkSlug(wine) === routeSlug(route.slug) || routeSlug(wine.id) === routeSlug(route.slug));
      if (!item) return;
  ...
  function navigateSection(nextView: SectionView, replace = false) {
    const route = routeForView(nextView);
    setPendingRoute(route);
    setView(nextView);
    setMobileNav(false);
    setEditingWine(null);
    writeCentralPath(sectionPath(nextView), replace);
  }
  ...
  function enterPortal(role: PortalRole) {
    ...
    setEntryStage('hub');
    if (!pendingRoute) {
      const defaultView: SectionView = role === 'distribution' ? 'distribution' : 'library';
      const route = routeForView(defaultView);
      setPendingRoute(route);
      setView(defaultView);
      writeCentralPath(sectionPath(defaultView));
    }
  }
  ...
  function switchPortal() {
    ...
    setEntryStage('roles');
    writeCentralPath(centralPath('/'));
  }

  function enterFromWelcome() {
    if (pendingRoute) {
      enterPortal(preferredPortalForRoute(pendingRoute, access.enabled ? access.role : null));
      return;
    }
    setEntryStage('roles');
  }
  ...
  function openWine(wine: WineRecord) {
    const route: CentralRoute = { kind: 'wine', slug: winePermalinkSlug(wine), tab: 'overview' };
    setPendingRoute(route);
    setActiveWineId(wine.id);
    setProfileTab('overview');
    setEditingWine(null);
    setView('profile');
    setMobileNav(false);
    writeCentralPath(wineProfilePath(wine));
  }

  function changeProfileTab(tab: ProfileTab) {
    if (!activeWine) return;
    const route: CentralRoute = { kind: 'wine', slug: winePermalinkSlug(activeWine), tab };
    setPendingRoute(route);
    setProfileTab(tab);
    writeCentralPath(wineProfilePath(activeWine, tab));
  }

  function openDistributionWine(wine: DistributionWine) {
    const route: CentralRoute = { kind: 'distribution', slug: distributionPermalinkSlug(wine) };
    setPendingRoute(route);
  ...
  function openTech(wine = activeWine) {
    ...
    setView('tech');
    setMobileNav(false);
    writeCentralPath(techSheetPath(wine));
  }

  function selectTechWine(id: string) {
  ...
  if (entryStage === 'welcome') {
    return <WelcomePage onEnter={enterFromWelcome} />;
  }
    ...
    <div className="app-shell min-h-screen bg-white text-[#111]">
      ...
      <main className="min-h-screen lg:pl-[292px]">
        ...
        {view === 'distribution-profile' && canUseDistribution && activeDistributionWine && (
          <DistributionWineDetail item={activeDistributionWine} wines={wines} back={() => navigateSection('distribution')} canEdit={isPortalAdmin} saveItem={saveDistributionWine} />
        )}
        ...
        {view === 'tech' && canUseTechSheets && (
          <TechSheetBuilder wines={wines} distributionWines={distributionWines} activeWine={activeWine} activeWineId={activeWineId} setActiveWineId={selectTechWine} draft={techDraft} setDraft={setTechDraft} back={() => navigateSection('tech-library')} />
        )}
    ...
    <div className="mb-6 overflow-hidden rounded-[28px] border border-black/10 bg-white shadow-sm">
      <div className="grid lg:grid-cols-[320px_1fr]">
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-2"><CopyLinkButton path={distributionWinePath(item)} label="Copy product link" /></div>
  ...
  return <div className="tech-builder flex min-h-screen flex-col bg-[#dfe2e6] xl:h-screen xl:min-h-0 xl:overflow-hidden">
    <div className="no-print flex flex-col border-b border-black/10 bg-white px-4 py-3 xl:flex-row xl:items-center xl:justify-between xl:px-6">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 xl:mt-0">
        ...
        {activeWine && <CopyLinkButton path={techSheetPath(activeWine)} label="Copy tech sheet link" className="flex items-center gap-2 rounded-lg border border-black/10 bg-white px-3 py-2 text-xs font-bold text-black/65 hover:border-black/20 hover:text-black" />}
